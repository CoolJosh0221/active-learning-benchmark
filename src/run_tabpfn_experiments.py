...
DATASETS = {
    ...
    "pol": {"variance": 16.07, "us_advantage": 2.84, "description": "Political data"},
}
...
class ExperimentRunner:
    ...
    def run_single_experiment(self, dataset, strategy_name, seed):
        ...
        cmd = [
            ...
            "1",
            "--init_lbl_size",
            str(self.config["init_lbl_size"]),
            "--tst_size",
            str(self.config["tst_size"]),
            "--total_budget",
        ...
        try:
            ...
            self.log(
                f"✗ Timeout: {strategy_name} on {dataset} (seed={seed})", level="ERROR"
            )
            return False
    ...
    def run_all_experiments(self, datasets=None, strategies=None):
        ...
        self.log(
            f"Total experiments: {len(datasets) * len(strategies) * self.config['n_trials']}"
        )
        self.log("=" * 80)
    ...
    def print_summary(self, total, successful):
        ...
        for dataset in DATASETS:
            ...
            if dataset_results:
                ...
                self.log(
                    f"  {dataset}: {dataset_success}/{dataset_total} ({rate:.1f}%)"
                )
    ...
    def save_results(self):
        ...
        results_file = (
            self.output_dir
            / f"tabpfn_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
def main():
    ...
    parser.add_argument(
        "--budget", type=int, default=200, help="Labeling budget (default: 200)"
    )
